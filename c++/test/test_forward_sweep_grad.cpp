...
TEMPLATE_LIST_TEST_CASE("check the results of forward_sweep_grad", "[forward_sweep_grad]", TestKernels) {
  ...
  celerite2::core::internal::forward_sweep<is_solve, true>(U, P, W, Y, Z, F);
  ...
  for (int n = 0; n < N; ++n) {
    ...
    for (int j = 0; j < J; ++j) {
      Z0 = Y;
      U0(n, j) += eps;
      celerite2::core::internal::forward_sweep<is_solve, true>(U0, P0, W0, Y0, Z0, F0);
      U0(n, j) -= eps;
      dZdU[n][j] = (Z0 - Z) / eps;
  ...
  for (int n = 0; n < N; ++n) {
    for (int k = 0; k < nrhs; ++k) {
      ...
      bW.setZero();
      bY.setZero();
      celerite2::core::internal::forward_sweep_grad<is_solve>(U, P, W, Y, Z, F, bZ, bU, bP, bW, bY);
      for (int m = 0; m < N; ++m) {
        for (int j = 0; j < J; ++j) {
